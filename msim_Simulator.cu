...
#ifndef FAKE_MULTI_GPU
	isPeerAccess=cudaDeviceCanAccessPeer(&os.nAccessEnable,mGPUID,pTarget->mGPUID);
	if(os.nAccessEnable)
	{
		//printf("communication from GPU%d to GPU%d enable!\n",mGPUID,pTarget->mGPUID);
		isPeerAccess=cudaDeviceEnablePeerAccess(pTarget->mGPUID,0);
...
bool Simulator::Prepare(SFNum timestep,SNum nTSGroupLen,SNum nTSMaxDelay,SNum nBlockSize)
{
	...
	mBlockSize=deviceProp.maxThreadsPerBlock;
...
bool Simulator::Simulate(bool bSelectActiveNeuron)
{
	...
	if(currentPos==0 && mNowTSGroup>0 && mCurrentGroupCount>0)//重置仿真过程的数据
	{
		...
		if(isPeerAccess){
			...
			int size1=sizeof(SFNum)*mNodeOffsets[TYPE_COUNT]*mMaxTSDelay;
			SFNum space[size1];
			CUDACHECK(cudaMemcpy(space,&mgCurrent[mNodeOffsets[TYPE_COUNT]*mCurrentGroupCount*mGroupLength],\
				size1,cudaMemcpyDeviceToHost));
			CUDACHECK(cudaMemcpy(mgCurrent,&space,\
		...
		for(SNum i=0;i<(SNum)mOutputSyn.size();i++)
		{
			...
			if(isPeerAccess){
				...
				int size2=sizeof(SFNum)*os.nDstNode*mMaxTSDelay;
				SFNum space2[size2];
				CUDACHECK(cudaMemcpy(space2,&os.gCurrentBuffer[os.nDstNode*mGroupLength*mCurrentGroupCount],\
					size2,cudaMemcpyDeviceToHost));
				CUDACHECK(cudaMemcpy(os.gCurrentBuffer,&space2,\
